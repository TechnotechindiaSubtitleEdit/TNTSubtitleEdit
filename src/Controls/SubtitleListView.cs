...
namespace Nikse.SubtitleEdit.Controls
{
    public sealed class SubtitleListView : ListView
    {
        ...
        private void Add(Paragraph paragraph)
        {
            ...
            if (value.IndexOf("\t") != -1)
            {
                item.SubItems.Add(paragraph.Text.Substring(paragraph.Text.IndexOf("\t")));
            }
            else if (value.IndexOf("{\\an5}") > 0)
            {
                item.SubItems.Add(paragraph.Text.Substring(paragraph.Text.LastIndexOf("}")));
            }
            else if (value.IndexOf("{\an5}") > 0)
            {
                item.SubItems.Add(paragraph.Text.Substring(paragraph.Text.LastIndexOf("}")));
            }
            else if (value.IndexOf("{\a5}") > 0)
            {
                item.SubItems.Add(paragraph.Text.Substring(paragraph.Text.LastIndexOf("}")));
            }
            else
        ...
        public void SetTimeAndText(int index, Paragraph paragraph)
        {
            if (IsValidIndex(index))
            {
                ...
                item.SubItems.Add(paragraph.Text.Substring(paragraph.Text.Replace(Environment.NewLine, _lineSeparatorString).IndexOf("\t")));
